...
def test_settings_use_postgres_uri_when_present() -> None:
    ...
    assert settings.runtime_postgres_uri == "postgresql://postgres:postgres@host.docker.internal:5432/postgres"


def test_settings_reject_insecure_auth_defaults_outside_development() -> None:
    with pytest.raises(ValueError, match="AUTH_SECRET_KEY must be changed outside development"):
