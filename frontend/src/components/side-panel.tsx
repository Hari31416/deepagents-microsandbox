...
import { ArtifactViewer } from "./chat/artifact-viewer"

export function SidePanel() {
  const { activeThreadId, threadFiles, setThreadFiles, isWorkspaceOpen, toggleWorkspace, setSelectedFile } = useStore()
              ...
              <div className="space-y-3">
                {files.filter(f => f.purpose === "upload").length === 0 ? (
                  ...
                  files.filter(f => f.purpose === "upload").map(file => (
                    <FileItem key={file.file_id} file={file} onDownload={() => handleDownload(file)} onPreview={() => setSelectedFile(file)} icon={getFileIcon(file.content_type)} />
                  ))
                )}
              </div>
            </ScrollArea>
...
function FileItem({ file, onDownload, onPreview, icon }: { file: ThreadFile, onDownload: (e: React.MouseEvent) => void, onPreview: () => void, icon: React.ReactNode }) {
  return (
    <div 
      className="group flex items-center justify-between p-3 rounded-2xl border border-slate-100 dark:border-slate-800/50 hover:border-primary/20 hover:bg-white dark:hover:bg-slate-900/60 transition-all shadow-sm hover:shadow-md mx-0.5 cursor-pointer"
      onClick={onPreview}
    >
